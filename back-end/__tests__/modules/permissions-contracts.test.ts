...
const fiscalResourceSource = readModule('back-end/modules/fiscal/fiscal.resource.ts');
...
test('front-end mantem custos operacionais em Operacao e contas a pagar em Gestao', () => {
  ...
  assert.match(sidebarSource, /id: 'management'[\s\S]*navItem\('fiscal', 'Fiscal'/);
});
...
test('front-end permite leitura financeira ampla e protege acoes por perfil', () => {
  ...
  assert.match(frontPermissionsSource, /fiscal:\s*\{[\s\S]*read: \['dev', 'owner', 'admin', 'financial', 'operational', 'viewer'\]/);
  assert.match(frontPermissionsSource, /fiscal:\s*\{[\s\S]*create: \['dev', 'owner', 'admin', 'financial'\]/);
...
test('back-end mantem custos operacionais e leitura financeira alinhados ao menu', () => {
  ...
  assert.match(fiscalResourceSource, /read: \['dev', 'owner', 'admin', 'financial', 'operational', 'viewer'\]/);
  assert.match(fiscalResourceSource, /create: \['dev', 'owner', 'admin', 'financial'\]/);
  assert.match(fiscalResourceSource, /update: \['dev', 'owner', 'admin', 'financial'\]/);
  assert.match(fiscalResourceSource, /delete: \['dev', 'owner', 'admin'\]/);
});
